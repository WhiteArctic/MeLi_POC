...
class DetectorTest(unittest.TestCase):
    ...
    def test_public_output_is_readable_without_internal_metadata(self):
        service = ImageModerationService()
        diagnosis = service.diagnose_text("Oferta especial con envio gratis")
        output = diagnosis.public_dict()

        self.assertEqual(
            set(output),
            {"has_infraction", "decision", "score", "evidence", "ocr_text", "signals"},
        )
        self.assertEqual(output["decision"], "infraction_detected")
        self.assertIn("rules", output["signals"])
        self.assertNotIn("matched_terms", output)
        self.assertNotIn("rule_score", output)
